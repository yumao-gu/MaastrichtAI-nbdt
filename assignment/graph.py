...
import json
import random
from assignment.utils import DATASETS, METHODS, fwd
...
def get_parser():
    ...
    parser.add_argument('--fname', type=str, choices=FNAME,
    ...
    parser.add_argument('--checkpoint', type=str, choices=CHECKPOINT, default='./mnist_cnn.pth',
    ...
    parser.add_argument('--induced-linkage', type=str, default='ward',
        help='(induced hierarchy) Linkage type used for agglomerative clustering "ward", "complete", "average", "single"')
    parser.add_argument('--induced-affinity', type=str, default='euclidean',
        help='(induced hierarchy) Metric used for computing similarity "l1", "l2", "manhattan", "cosine","euclidean"')
...
def generate_fname(method, seed=0, branching_factor=2, extra=0,no_prune=False, fname='', multi_path=False,induced_linkage='ward', induced_affinity='euclidean',checkpoint=None, arch=None, **kwargs):
    return fname

def get_directory(dataset, root='./'):
    return './'
...
def get_graph_path_from_args(dataset, method, seed=0, branching_factor=2, extra=0,no_prune=False, fname='', multi_path=False,induced_linkage='ward', induced_affinity='euclidean',checkpoint=None, arch=None, **kwargs):
...
MODEL_FC_KEYS = (
    ...
    'output.fc.weight', 'module.output.fc.weight', 'classifier.weight','fc2.weight')


def build_induced_graph(wnids, checkpoint, model=None, linkage='ward',affinity='euclidean', branching_factor=2, dataset='CIFAR10',state_dict=None):
    num_classes = len(wnids)
    assert checkpoint or model or state_dict, \
        'Need to specify either `checkpoint` or `method` or `state_dict`.'
    if state_dict:
        print("mnist induced graph use the state_dict channel")
        centers = get_centers_from_state_dict(state_dict)
    elif checkpoint:
        print("mnist induced graph use the checkpoint channel")
        centers = get_centers_from_checkpoint(checkpoint)
    else:
        print("mnist induced graph close the model channel")
